...
class CowDataset(D.Dataset):
    ...
    def __init__(self):
        ...
        self.class_num = len(label_dic_1.keys())
        self.load()
...
model = smp.Unet(
    ...
    classes=len(label_dic_1.keys()),                      # model output channels (number of classes in your dataset)
)
model.to(DEVICE)
...
for epoch in tqdm(range(EPOCHES)):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), '/root/code/model_state/unet_body_0209.pth')
